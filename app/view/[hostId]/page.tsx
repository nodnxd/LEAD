...
type PitchFileItem = {
  ...
  vocal: 'male'|'female'|'unknown'; duration: number;
  analyzing: boolean; isDuplicate: boolean;
  bpm: string; genre: string;
};
...
export default function GuestView(){
  ...
  const submitPitch=async()=>{
    ...
    // 1. pitches 레코드 생성
    const {data:pitchData,error:pitchErr}=await supabase.from('pitches').insert({
    ...
    if(pitchErr||!pitchData){
      ...
      setUploadError(`피칭 등록 실패: ${pitchErr?.message||'알 수 없는 오류'}`);
      return;
    }
    ...
    if(pitchFiles.length>0){
      ...
      for(let i=0;i<total;i++){
        ...
        const {data:upData,error:upErr}=await supabase.storage
          .from('pitch-files')
          .upload(path,pf.file,{contentType:'audio/mpeg',upsert:false});

        if(upErr){
          setPitchLoading(false);
          setUploadError(`파일 업로드 실패: ${upErr.message}`);
          return;
        }

        const fileUrl=supabase.storage.from('pitch-files').getPublicUrl(path).data.publicUrl;

        const {error:pfErr}=await supabase.from('pitch_files').insert({
          pitch_id:pitchData.id,
          host_id:hostId,
          file_url:fileUrl,
          file_name:pf.file.name,
          file_hash:pf.hash||null,
          bpm:pf.bpm?parseInt(pf.bpm)||null:null,
          vocal_gender:pf.vocal||null,
          genre:pf.genre||null,
        ...
        if(pfErr){
          setPitchLoading(false);
          setUploadError(`파일 정보 저장 실패: ${pfErr.message}`);
          return;
        }
  ...
  const renderContent=(content:string)=>{if(!content)return null;return content.split(/(https?:\/\/[^\s]+)/g).map((part,i)=>{if(part.match(/^https?:\/\//))return<a key={i} href={part} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[#5B8CFF] hover:underline break-all"><span>{getLinkIcon(part)}</span><span>{part.replace(/^https?:\/\//,'').split('/').slice(0,2).join('/')}</span></a>;return<span key={i} className="whitespace-pre-wrap">{part}</span>;});};
    ...
    <div className="border border-white/10 rounded-2xl bg-white/[0.03] overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-zinc-600 text-[11px]">{(item.file.size/1024/1024).toFixed(1)}MB{item.duration>0&&` · ${fmtDur(item.duration)}`}</p>
        </div>
        ...
        <div className="px-4 pb-4 border-t border-white/5 pt-3 flex flex-col gap-3">
          ...
          {item.vocal!=='unknown'&&(
            <div className="flex items-center gap-2">
              <span className="text-zinc-600 text-[10px] font-black uppercase tracking-widest">보컬</span>
              <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${vocalCls(item.vocal)}`}>🎤 {vocalLabel(item.vocal)}</span>
              <span className="text-zinc-700 text-[10px]">자동 감지</span>
            </div>
          )}

          {/* BPM 수동 입력 */}
          <div className="flex items-center gap-3">
            <label className="text-zinc-600 text-[10px] font-black uppercase tracking-widest shrink-0">BPM</label>
            <input
              type="number" min="40" max="300"
              value={item.bpm}
              onChange={e=>updateFile(item.id,{bpm:e.target.value})}
              placeholder="직접 입력 (예: 120)"
              className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-[12px] outline-none focus:border-[#5B8CFF]/50 transition-all placeholder:text-zinc-700 text-white"
            />
          </div>

          {/* 장르 */}
          <div>
            <p className="text-zinc-600 text-[10px] font-black uppercase tracking-widest mb-2">장르</p>
            ...
            <div className="p-6">
              {pitchSent?(
                <div className="text-center py-10">
                  ...
                  <p className="text-zinc-400 text-[12px] mt-1">{pitchForm.artist_name}{pitchFiles.length>0&&` · 파일 ${pitchFiles.length}개`}</p>
                ...
                <>
                  <div className="mb-5"><h2 className="text-white font-black text-[20px]">🎵 피칭하기</h2><p className="text-zinc-500 text-[12px] mt-0.5">{pitchingLead.artist} — {pitchingLead.title}</p></div>
                  <div className="flex flex-col gap-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div><label className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-1.5 block">아티스트명 *</label><input value={pitchForm.artist_name} onChange={e=>setPitchForm(p=>({...p,artist_name:e.target.value}))} placeholder="아티스트명" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-[13px] outline-none focus:border-[#5B8CFF]/50 transition-all placeholder:text-zinc-700 text-white"/></div>
                      <div><label className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-1.5 block">연락처 *</label><input value={pitchForm.contact} onChange={e=>setPitchForm(p=>({...p,contact:e.target.value}))} placeholder="이메일/전화/카카오" className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-[13px] outline-none focus:border-[#5B8CFF]/50 transition-all placeholder:text-zinc-700 text-white"/></div>
                    </div>

                    {/* 파일 업로드 */}
                    <div>
                      <div className="flex items-center justify-between mb-2"><label className="text-zinc-500 text-[10px] font-black uppercase tracking-widest">데모 파일 <span className="text-zinc-700 font-normal normal-case">MP3 · 최대 50MB</span></label>{pitchFiles.length>0&&<span className="text-zinc-600 text-[11px]">{pitchFiles.length}개</span>}</div>
                      ...
                      <div onDragOver={e=>e.preventDefault()} onDrop={handleFileDrop} onClick={()=>fileInputRef.current?.click()} className="border-2 border-dashed border-white/10 rounded-2xl p-4 text-center cursor-pointer hover:border-white/20 hover:bg-white/[0.02] transition-all">
                        <p className="text-zinc-600 text-[12px] font-bold">+ 파일 추가</p>
                        <p className="text-zinc-800 text-[11px] mt-0.5">클릭 또는 드래그 · 여러 개 동시 선택 가능</p>
                      </div>
                    </div>

                    <div><label className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-1.5 block">메시지 <span className="text-zinc-700 font-normal normal-case">(선택)</span></label><textarea value={pitchForm.message} onChange={e=>setPitchForm(p=>({...p,message:e.target.value}))} placeholder="한마디, 포트폴리오 링크 등" rows={2} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[13px] outline-none focus:border-[#5B8CFF]/50 transition-all placeholder:text-zinc-700 text-white resize-none leading-relaxed"/></div>
                  </div>

                  {pitchLoading&&<div className="mt-4"><div className="flex items-center justify-between mb-1.5"><span className="text-zinc-500 text-[11px]">업로드 중...</span><span className="text-zinc-400 text-[11px] font-bold">{uploadProgress}%</span></div><div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-[#3B6FFF] to-[#7BA4FF] rounded-full transition-all" style={{width:`${uploadProgress}%`}}/></div></div>}
                  ...
                  <div className="flex gap-3 mt-5">
                    ...
                    <button onClick={submitPitch} disabled={pitchLoading||!pitchForm.artist_name.trim()||!pitchForm.contact.trim()||pitchFiles.some(f=>f.analyzing)}
