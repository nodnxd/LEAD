...
export default function GuestView(){
  ...
  const LeadCard=({lead,compact=false}:{lead:any;compact?:boolean})=>{
    ...
    return(
      <div onClick={(e)=>{e.stopPropagation();setViewingLead(lead);setContentLang('ko');}} className={`border rounded-2xl cursor-pointer transition-all hover:scale-[1.02] ${c.bg} ${c.border} ${expired?'opacity-40 grayscale':''} ${compact?'p-2':'p-4'}`}>
    ...
    <>
      ...
      <main className={`min-h-screen ${mainBg} flex items-center justify-center p-5 font-pretendard relative overflow-hidden`}>
        ...
        <div className="w-full max-w-sm text-center">
          <div className="flex flex-col items-center mb-10"><div className="flex items-baseline justify-center gap-2.5"><h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#5B8CFF] to-[#a5c0ff] uppercase tracking-tighter">LEAD</h1><span className={`${dimText} text-[11px] font-bold tracking-[0.2em]`}>by NEN</span></div>{hostCompany&&<div className="mt-2 flex items-center gap-1.5 px-3 py-1 rounded-full border border-[#5B8CFF]/25 bg-[#5B8CFF]/10"><span className="text-[10px]">🏢</span><span className={`text-[12px] font-black ${D?'text-zinc-200':'text-zinc-700'}`}>{hostCompany}</span></div>}</div>
    ...
    <>
      ...
      <main className={`min-h-screen ${mainBg} p-5 lg:p-8 font-pretendard relative`}>
        ...
        <div className="relative z-10 flex flex-col items-center mb-8">
          ...
          {hostCompany&&<div className="mt-2 flex items-center gap-1.5 px-3 py-1 rounded-full border border-[#5B8CFF]/25 bg-[#5B8CFF]/10"><span className="text-[10px]">🏢</span><span className={`text-[12px] font-black ${D?'text-zinc-200':'text-zinc-700'}`}>{hostCompany}</span></div>}
        </div>
            ...
            <div className="p-6">
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><label className={`text-[10px] font-black uppercase tracking-widest mb-1.5 block ${D?'text-zinc-500':'text-zinc-400'}`}>곡 제목</label><input value={lTitle} onChange={e=>setLTitle(e.target.value)} placeholder="곡 제목 (선택)" className={`w-full border rounded-xl px-3 py-2.5 text-[13px] outline-none transition-all ${inputCls}`}/></div>
                </div>
                <div>
