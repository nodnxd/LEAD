...
export default function HubPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#141414] flex items-center justify-center"><div className="w-6 h-6 border-2 border-[#80A1D4] border-t-transparent rounded-full animate-spin" /></div>
  );
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#141414] text-white font-pretendard p-5 lg:p-8 relative overflow-hidden">
        <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] rounded-full pointer-events-none" style={{background:'#80A1D4', filter:'blur(200px)', animation:'orb-pulse 4s ease-in-out infinite'}} />
        <div className="relative z-10 max-w-xl mx-auto">
          <div className="flex items-center justify-between mb-10">
            <div className="flex items-baseline gap-2.5">
              <h1 className="text-3xl font-semibold text-[#80A1D4] uppercase tracking-tighter">LEAD</h1>
              ...
              <div className="flex flex-col gap-2.5">
                {member.map(c => (
                  <button key={c.id} onClick={() => enterMember(c.id)} className="flex items-center gap-3 p-4 rounded-2xl border border-white/[0.07] bg-white/[0.02] hover:bg-white/[0.05] text-left transition-all">
                    <div className="w-11 h-11 rounded-xl bg-[#80A1D4]/15 flex items-center justify-center text-[18px] shrink-0">🎤</div>
          ...
          {canHost && (
            <section>
              ...
              <div className="flex flex-col gap-2.5">
                {operate.map(c => (
                  <button key={c.id} onClick={() => enterOperate(c.id)} className="flex items-center gap-3 p-4 rounded-2xl border border-[#80A1D4]/30 bg-[#80A1D4]/[0.07] hover:bg-[#80A1D4]/[0.12] text-left transition-all">
                    <div className="w-11 h-11 rounded-xl bg-[#80A1D4]/15 flex items-center justify-center text-[18px] shrink-0">🏢</div>
          ...
          <div className="mt-10 pt-6 border-t border-white/[0.07]">
            <a href="/roster/dashboard" className="flex items-center gap-3 p-4 rounded-2xl border border-[#7F5A83]/25 bg-[#7F5A83]/[0.06] hover:bg-[#7F5A83]/10 text-left transition-all">
              <div className="w-11 h-11 rounded-xl bg-[#7F5A83]/15 flex items-center justify-center text-[18px] shrink-0">🎤</div>
              ...
              <span className="text-[#7F5A83] text-[18px] font-black shrink-0">→</span>
            </a>
          </div>
        </div>
      </main>
