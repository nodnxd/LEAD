              ...
              <div className="flex flex-col gap-2.5">
                {operate.map(c => (
                  <button key={c.id} onClick={() => enterOperate(c.id)} className="flex items-center gap-3 p-4 rounded-2xl border border-amber-500/25 bg-amber-500/[0.06] hover:bg-amber-500/10 text-left transition-all">
                    <div className="w-11 h-11 rounded-xl bg-amber-500/15 flex items-center justify-center text-[18px] shrink-0">🏢</div>
                    ...
                    <span className="text-amber-400 text-[18px] font-black shrink-0">→</span>
                  </button>
                ))}
              </div>
            </section>
