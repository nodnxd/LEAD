    ...
    <>
      ...
      <main className={`min-h-screen ${bg} font-pretendard p-5 flex flex-col items-center justify-center`}>
        ...
        <div id="comp-card" className={`w-full max-w-sm border rounded-3xl overflow-hidden shadow-2xl ${card}`}>
          {/* 헤더 배너 */}
          <div className={`h-24 relative ${isHost ? 'bg-gradient-to-br from-amber-500/40 to-orange-400/20' : 'bg-gradient-to-br from-[#3E78DB]/40 to-purple-500/20'}`}>
            <div className="absolute bottom-[-36px] left-6">
              <div className="rounded-2xl overflow-hidden border-2 border-white/20 bg-black/30 flex items-center justify-center shadow-xl" style={{ width: 72, height: 72 }}>
                {showPhoto
                  ...
                  : <span className={`text-3xl font-black ${isHost ? 'text-amber-400' : 'text-[#3E78DB]'}`}>{displayName[0].toUpperCase()}</span>}
              </div>
            </div>
            {isHost && (
            ...
            {(profile.roles || []).length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2.5">
                {profile.roles.map((r: string) => (
                  <span key={r} className={`text-[11px] font-black px-2.5 py-1 rounded-full border ${isHost ? 'border-amber-500/30 bg-amber-500/10 text-amber-400' : 'border-[#3E78DB]/30 bg-[#3E78DB]/10 text-[#3E78DB]'}`}>
                    {ROLE_LABELS[r] || r}
                  </span>
                ))}
              </div>
                ...
                <div className="flex flex-col gap-1.5">
                  {works.slice(0, 5).map((w, i) => (
                    <a key={i} href={w.link} target="_blank" rel="noopener noreferrer"
                      ...
                      <span className="text-[#3E78DB] text-[12px]">→</span>
                    </a>
                  ))}
                </div>
              </div>
