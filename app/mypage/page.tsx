...
export default function MyPage() {
  ...
  const inputCls = D
    ? 'bg-white/[0.04] border-white/[0.08] text-white placeholder:text-zinc-500 focus:border-[#80A1D4]/60'
  ...
  if (loading || userType === 'loading') return (
    <div className={`min-h-screen ${bg} flex items-center justify-center`}>
      <div className="w-6 h-6 border-2 border-[#80A1D4] border-t-transparent rounded-full animate-spin" />
    </div>
  );
    ...
    return (
      <>
        ...
        <main className={`min-h-screen ${bg} font-pretendard p-5 lg:p-8 relative overflow-hidden`} style={{zoom:1.1}}>
          {D && <div className="absolute top-[-15%] right-[-10%] w-[500px] h-[500px] rounded-full pointer-events-none" style={{background:'#80A1D4',filter:'blur(180px)',animation:'orb-pulse 4s ease-in-out infinite'}} />}

          <div className="relative z-10 max-w-2xl mx-auto">
            {/* 헤더 */}
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-baseline gap-2.5">
                <h1 className="text-2xl font-semibold text-[#80A1D4] uppercase tracking-tighter">LEAD</h1>
              ...
              {(hostProfile?.roles?.length > 0 || hostProfile?.instagram || hostProfile?.bio) && (
                <div className={`px-5 pb-4 border-t ${D ? 'border-white/[0.06]' : 'border-black/[0.06]'} pt-3`}>
                  ...
                  {hostProfile?.instagram && (
                    <a href={`https://instagram.com/${hostProfile.instagram}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-1 text-[11px] text-[#80A1D4] hover:underline">📸 @{hostProfile.instagram}</a>
                  )}
                </div>
              )}
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: '전체 리드', value: hostLeads.length, sub: `활성 ${activeLeads.length}`, color: 'text-[#80A1D4]' },
                ...
                {(['pitches', 'members'] as const).map(tab => (
                  <button key={tab} onClick={() => setHostTab(tab)}
                    className={`flex-1 py-3.5 text-[12px] font-black transition-all ${hostTab === tab
                      ? D ? 'text-white border-b-2 border-[#80A1D4]' : 'text-[#111] border-b-2 border-[#80A1D4]'
                      : dimText}`}>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {([['recent','최신순'],['bpm','BPM'],['vocal','보컬'],['key','Key']] as const).map(([v,l])=>(
                        <button key={v} onClick={()=>setPitchSort(v)} className={`text-[10px] font-black px-2.5 py-1 rounded-full border transition-all ${pitchSort===v?'bg-[#80A1D4] border-[#80A1D4] text-white':D?'border-white/10 text-zinc-500 hover:text-white':'border-black/[0.08] text-zinc-500 hover:text-[#111]'}`}>{l}</button>
                      ))}
                    </div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {([['all','전체'],['male','남성'],['female','여성'],['both','혼성']] as const).map(([v,l])=>(
                        <button key={v} onClick={()=>setPitchVocalFilter(v)} className={`text-[10px] font-black px-2.5 py-1 rounded-full border transition-all ${pitchVocalFilter===v?'bg-[#80A1D4] border-[#80A1D4] text-white':D?'border-white/10 text-zinc-500 hover:text-white':'border-black/[0.08] text-zinc-500 hover:text-[#111]'}`}>{l}</button>
                      ))}
                    </div>
                  </div>
                  {(() => {
                    ...
                    : hostMembers.map((a: any) => {
                      ...
                      return (
                        <div key={a.id} className={`flex items-center gap-3 p-3 rounded-xl border ${D ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-black/[0.02] border-black/[0.06]'}`}>
                          <div className="w-9 h-9 rounded-full overflow-hidden bg-[#80A1D4]/10 border border-[#80A1D4]/20 flex items-center justify-center shrink-0">
                            {m?.photo_url
                              ? <img src={m.photo_url} alt="" className="w-full h-full object-cover" />
                              : <span className="text-[13px] font-black text-[#80A1D4]">{(m?.artist_name || '?')[0].toUpperCase()}</span>
                            }
                          </div>
                          <div className="flex-1 min-w-0">
          ...
          {hostEditing && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md font-pretendard p-4 overflow-y-auto" onClick={() => setHostEditing(false)}>
                ...
                <div className="p-6 max-h-[85vh] overflow-y-auto">
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    <button onClick={saveHostProfile} disabled={hostSaving}
                      className="flex-1 py-3 rounded-xl bg-[#80A1D4] text-white font-semibold text-[13px] hover:opacity-90 transition-all disabled:opacity-50">
                      {hostSaving ? '저장 중...' : '저장'}
                    </button>
                  </div>
                </div>
    ...
    <>
      ...
      <main className={`min-h-screen ${bg} font-pretendard p-5 lg:p-8 relative overflow-hidden`} style={{zoom:1.1}}>
        ...
        <div className="relative z-10 max-w-2xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-baseline gap-2.5">
              <h1 className="text-2xl font-black text-[#80A1D4] uppercase tracking-tighter">LEAD</h1>
          ...
          <div className={`border rounded-2xl overflow-hidden mb-5 ${card}`}>
            <div className={`p-6 border-b ${divider}`}>
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full overflow-hidden bg-[#80A1D4]/10 border border-[#80A1D4]/20 flex items-center justify-center shrink-0">
                  {member?.photo_url
                    ? <img src={member.photo_url} alt="" className="w-full h-full object-cover" />
                    : <span className="text-2xl font-black text-[#80A1D4]">{(member?.artist_name || '?')[0].toUpperCase()}</span>
                  }
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {member?.roles?.map((r: string) => (
                      <span key={r} className="text-[9px] font-black px-2 py-0.5 rounded-full bg-[#80A1D4]/10 border border-[#80A1D4]/20 text-[#80A1D4]">
                        {roleLabels[r] || r}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-1.5 shrink-0">
                  <button onClick={openEdit}
                    className="px-3 py-2 rounded-xl bg-[#80A1D4]/10 border border-[#80A1D4]/20 text-[#80A1D4] text-[11px] font-bold hover:bg-[#80A1D4]/20 transition-all">
                    ✏️ 수정
                  </button>
                  <a href={`/card/${user?.id}`} target="_blank" rel="noopener noreferrer"
            ...
            <div className={`px-6 py-4 border-b ${divider} flex flex-wrap gap-4`}>
              ...
              {member?.instagram && (
                <div>
                  ...
                  <a href={`https://instagram.com/${member.instagram}`} target="_blank" rel="noopener noreferrer" className="text-[13px] text-[#80A1D4] hover:underline">@{member.instagram}</a>
                </div>
              )}
            </div>
                ...
                <div className="flex flex-col gap-2">
                  {demos.map(d => (
                    <div key={d.id} className={`flex items-center gap-3 p-3 rounded-xl border ${D ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-black/[0.02] border-black/[0.06]'}`}>
                      ...
                      }} className="text-[#80A1D4] text-[11px] font-bold hover:underline">⬇ 다운</button>
                    </div>
                  ))}
                </div>
              ) : <p className={`text-[12px] ${dimText}`}>등록된 데모곡이 없어요</p>}
              {member?.demo_link && (
                <a href={member.demo_link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-2 text-[11px] text-[#80A1D4] hover:underline">🔗 추가 데모 링크</a>
              )}
            </div>
                ...
                <div className="flex flex-col gap-2">
                  {works.map((w, i) => (
                    <a key={i} href={w.link} target="_blank" rel="noopener noreferrer"
                      className={`flex items-center gap-3 p-3 rounded-xl border transition-all hover:border-[#80A1D4]/30 ${D ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-black/[0.02] border-black/[0.06]'}`}>
                      <div className="w-8 h-8 rounded-lg bg-[#80A1D4]/10 border border-[#80A1D4]/20 flex items-center justify-center shrink-0">
                        <span className="text-[12px]">🎶</span>
                      </div>
                      <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[#80A1D4] text-[11px]">→</span>
                    </a>
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-4">
                    <button onClick={() => photoRef.current?.click()} className="relative group">
                      <div className="w-16 h-16 rounded-full overflow-hidden bg-[#80A1D4]/10 border-2 border-dashed border-[#80A1D4]/30 flex items-center justify-center">
                    ...
                    <div className="flex gap-2">
                      {[['male', '남성'], ['female', '여성'], ['other', '기타']].map(([v, l]) => (
                        <button key={v} onClick={() => setGender(v)}
                          className={`flex-1 py-2 rounded-xl border text-[12px] font-bold transition-all ${gender === v ? 'bg-[#80A1D4]/20 border-[#80A1D4]/50 text-[#80A1D4]' : D ? 'bg-white/[0.03] border-white/[0.08] text-zinc-500' : 'bg-black/[0.03] border-black/[0.08] text-zinc-500'}`}>
                          {l}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {ROLES.map(r => (
                        <button key={r.id} onClick={() => toggleArr(roles, r.id, setRoles)}
                          className={`px-3 py-1.5 rounded-xl border text-[11px] font-bold transition-all ${roles.includes(r.id) ? 'bg-[#80A1D4]/20 border-[#80A1D4]/50 text-[#80A1D4]' : D ? 'bg-white/[0.03] border-white/[0.08] text-zinc-500' : 'bg-black/[0.03] border-black/[0.08] text-zinc-500'}`}>
                          {r.label}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {GENRES.map(g => (
                        <button key={g.id} onClick={() => toggleArr(genres, g.id, setGenres)}
                          className={`px-3 py-1.5 rounded-xl border text-[11px] font-bold transition-all ${genres.includes(g.id) ? 'bg-[#80A1D4]/20 border-[#80A1D4]/50 text-[#80A1D4]' : D ? 'bg-white/[0.03] border-white/[0.08] text-zinc-500' : 'bg-black/[0.03] border-black/[0.08] text-zinc-500'}`}>
                          {g.label}
                        </button>
                      ))}
                    </div>
                    ...
                    {newDemoFiles.length > 0 && (
                      <div className="flex flex-col gap-1 mt-2">
                        {newDemoFiles.map((f, i) => (
                          <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#80A1D4]/5 border border-[#80A1D4]/20">
                            <span className="text-[12px]">🎵</span>
                            <span className="flex-1 text-[11px] text-[#80A1D4] truncate">{f.name}</span>
