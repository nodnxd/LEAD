...
;
  ...
  const exportAllDays = async (format: 'jpeg' | 'pdf' = 'jpeg') => {
    ...
    try {
      ...
      // 헤더
      ctx.font = 'bold 28px system-ui, sans-serif'; ctx.fillStyle = '#F6A823';
      ...
      allDayData.forEach(({ dayLabel, teamData }, di) => {
        ...
        // Day 라벨
        ctx.font = 'bold 12px system-ui, sans-serif'; ctx.fillStyle = '#F6A823';
    ...
    <>
      ...
      <div style={{ transform: `scale(${zoom * 1.1})`, transformOrigin: 'top left', width: `${100 / (zoom * 1.1)}%`, minHeight: `${100 / (zoom * 1.1)}vh` }}>
        <main className={`min-h-screen ${bg} ${textMain} p-5 lg:p-8 font-pretendard relative overflow-hidden transition-colors duration-150`}>
          {theme === 'dark' && <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] rounded-full pointer-events-none" style={{background:'#F6A823',filter:'blur(200px)',animation:'orb-pulse 4s ease-in-out infinite'}} />}

          {/* 헤더 */}
          <div className="relative z-10 flex items-center justify-center gap-3 mb-8 flex-wrap">
            <div className="flex items-baseline gap-2.5">
              <h1 className="text-4xl font-semibold text-[#F6A823] uppercase tracking-tighter">CAST</h1>
            ...
            <div className={`flex gap-1 p-1 rounded-full border ${theme === 'light' ? 'border-black/[0.08] bg-black/[0.04]' : 'border-white/10 bg-white/5'}`}>
              ...
              <span className="px-3 py-1 rounded-full bg-[#F6A823] text-white text-[11px] font-normal">CAST</span>
            </div>
          </div>
          ...
          <DragDropContext onDragEnd={onDragEnd}>
                ...
                <div {...provided.droppableProps} ref={provided.innerRef} className="relative z-10 flex items-center gap-2 mb-6 overflow-x-auto pb-2 no-scrollbar">
                  {projects.map((p, index) => (
                    <Draggable key={p} draggableId={`proj-${p}`} index={index}>
                      {(provided) => (
                        <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                          className={`flex items-center rounded-full border transition-all overflow-hidden ${currentProject === p ? 'border-[#F6A823]/50 bg-gradient-to-r from-[#F6A823]/30 to-[#E97582]/10 shadow-[0_0_20px_rgba(222,60,75,0.3)]' : theme === 'light' ? 'border-black/10 bg-black/5' : 'border-white/10 bg-white/5'}`}>
                  ...
                  <button onClick={() => showPrompt(t.newRoster, t.rosterNamePlaceholder, '', async (n) => { if (n) { const next = [...projects, n]; setProjects(next); await saveProjectOrder(user.id, next); setCurrentProject(n); } setPromptModal(null); })}
                    className={`px-3 py-1.5 rounded-full font-bold text-[12px] border border-dashed hover:text-[#F6A823] transition-all ${theme === 'light' ? 'bg-black/5 border-black/10 text-zinc-500' : 'bg-white/5 border-white/10 text-zinc-400'}`}>+</button>
                </div>
              )}
            </Droppable>
            ...
            <header className={`relative z-10 mb-6 border-b pb-4 ${theme === 'light' ? 'border-black/10' : 'border-white/10'}`}>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  }} className={`border px-3 py-1.5 rounded-lg font-bold text-[11px] transition-all text-[#F6A823] ${theme === 'light' ? 'bg-black/5 border-black/10 hover:bg-black/10' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>{t.studio}</button>
                  ...
                  <button onClick={() => { setShowArtistPanel(p => !p); if (!showArtistPanel) fetchArtists(user); }}
                    className={`border px-3 py-1.5 rounded-lg font-normal text-[11px] transition-all ${showArtistPanel ? 'border-[#F6A823]/50 text-[#F6A823] bg-[#F6A823]/10' : btnBg}`}>{t.addFromArtists}</button>
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">
                <button onClick={() => votingOpen
                  ...
                  className={`px-4 py-1.5 rounded-xl font-normal text-[11px] transition-all border ${votingOpen ? 'bg-[#F6A823]/20 border-[#F6A823]/40 text-[#E97582] hover:bg-[#F6A823]/30' : btnBg}`}>
                ...
                <button onClick={() => showPrompt(t.randomMatch, t.teamCount, '2', async (v) => { const n = parseInt(v); setPromptModal(null); if (n > 0) await generateRandomRoster(n)(); })}
                  className="bg-[#F6A823] text-white px-5 py-1.5 rounded-xl font-normal text-[11px] hover:opacity-90 transition-all uppercase tracking-tighter">{t.random}</button>
            ...
            <div className="relative z-10 flex items-center gap-2 mb-6">
              {days.map(d => (
                <div key={d} className={`flex items-center rounded-full border overflow-hidden transition-all ${currentDay === d ? 'border-[#F6A823]/50 bg-gradient-to-r from-[#F6A823]/20 to-transparent' : theme === 'light' ? 'border-black/10 bg-black/5' : 'border-white/10 bg-white/5'}`}>
                  {editingDayName === d ? (
                    <input autoFocus value={dayNameInput} onChange={e => setDayNameInput(e.target.value)}
                      ...
                      className="bg-transparent px-3 py-1.5 text-[11px] font-bold outline-none text-[#F6A823] w-24" />
                  ) : (
                    <button onClick={() => setCurrentDay(d)} onDoubleClick={() => { setEditingDayName(d); setDayNameInput(dayNames[d] || `Day ${d}`); }}
                      className={`px-4 py-1.5 font-bold text-[11px] transition-all ${currentDay === d ? 'text-[#F6A823]' : textSub}`}>{getDayLabel(d)}</button>
                  )}
              ...
              <button onClick={addDay} className={`px-3 py-1.5 rounded-full font-bold text-[12px] border border-dashed hover:text-[#F6A823] transition-all ${theme === 'light' ? 'bg-black/5 border-black/10 text-zinc-500' : 'bg-white/5 border-white/10 text-zinc-400'}`}>{t.addDay}</button>
            </div>
                          ...
                          {campSessions.sort((a: any, b: any) => a.day_number - b.day_number).map((s: any) => (
                            <div key={s.id} className={`rounded-2xl border overflow-hidden ${theme === 'light' ? 'border-black/10 bg-black/[0.02]' : 'border-white/10 bg-white/[0.02]'}`}>
                              <div className="flex items-center justify-between p-4 cursor-pointer" onClick={() => setExpandedSession(expandedSession === s.id ? null : s.id)}>
                                <div className="flex items-center gap-3"><span className="text-[#F6A823] font-black text-[13px]">Day {s.day_number}</span>{s.memo && <span className={`text-[12px] truncate max-w-[200px] ${textSub}`}>{s.memo}</span>}</div>
                              ...
                              {expandedSession === s.id && s.roster && (
                                <div className={`px-4 pb-4 border-t pt-4 ${theme === 'light' ? 'border-black/5' : 'border-white/5'}`}>
                                  ...
                                  }} className="mb-3 px-3 py-1.5 rounded-lg bg-[#F6A823]/20 text-[#F6A823] text-[11px] font-bold border border-[#F6A823]/30 hover:bg-[#F6A823]/30 transition-all">
                                    {lang === 'ko' ? '⬆ 현재 로스터로 불러오기' : '⬆ Load to Current Roster'}
                                  </button>
                                  <div className="flex flex-wrap gap-4">
                                    {s.roster.map((t: any) => (<div key={t.team} className="flex-1 min-w-[150px]"><p className={`text-[10px] font-black uppercase tracking-widest mb-2 border-l-2 border-[#F6A823] pl-2 ${textSub}`}>{t.team}</p>{t.members.map((m: any, i: number) => (<div key={i} className="flex items-center gap-1.5 mb-1"><span className={`text-[12px] font-bold ${textMain}`}>{m.name}</span><span className="text-zinc-600 text-[9px] uppercase">{m.role.slice(0, 3)}</span></div>))}</div>))}
                                  </div>
                                </div>
                              )}
                            </div>
                            ...
                            {group.map(artist => {
                              ...
                              return (
                                <button key={artist.id} onClick={() => !inRoster && addArtistToRoster(artist)} disabled={inRoster}
                                  ...
                                  {inRoster ? <span className="text-[10px] text-zinc-600 font-black">✓</span> : <span className="text-[10px] text-[#F6A823] font-black">+</span>}
                                </button>
                              );
                            })}
                          </div>
                        ...
                        {(provided) => (
                          <div ref={provided.innerRef} {...provided.draggableProps} className="w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] xl:w-[calc(25%-18px)]">
                            <div className={`backdrop-blur-2xl border rounded-[2rem] p-6 min-h-[400px] shadow-2xl flex flex-col ${cardBg}`}>
                              <div {...provided.dragHandleProps} className="flex justify-between items-start mb-4 px-1 border-l-4 border-[#F6A823] pl-4 cursor-grab">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm font-pretendard">
          <div className={`w-full max-w-sm mx-4 border rounded-2xl p-8 shadow-2xl ${theme === 'light' ? 'bg-white border-black/10' : 'bg-[#111] border-white/10'}`}>
            <div className="text-center mb-6">
              <h1 className="text-4xl font-semibold text-[#F6A823] uppercase tracking-tighter mb-2">CAST</h1>
            ...
            <input autoFocus value={firstRosterName} onChange={e => setFirstRosterName(e.target.value)} onKeyDown={e => e.key === 'Enter' && createFirstRoster()}
              ...
              className={`w-full border rounded-xl px-4 py-3 text-[14px] outline-none focus:border-[#F6A823]/50 transition-all mb-4 ${inputBg} ${textMain} placeholder:text-zinc-500`} />
            <button onClick={createFirstRoster} disabled={!firstRosterName.trim()}
              className="w-full py-3 rounded-xl bg-[#F6A823] text-white font-semibold text-[13px] uppercase tracking-widest hover:opacity-90 transition-all disabled:opacity-40">{t.start}</button>
          </div>
        </div>
      )}
      ...
      {showVotingModal && (
        <Modal title={t.voteOpenTitle} theme={theme}>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={openVoting} className="flex-1 py-3 rounded-xl bg-[#F6A823]/20 border border-[#F6A823]/40 text-[#E97582] font-black text-[12px] hover:bg-[#F6A823]/30 transition-all">{t.voteStart}</button>
          </div>
        </Modal>
      )}
      ...
      <div className="flex fixed bottom-6 left-6 z-50 flex-col items-center gap-1.5 select-none">
        ...
        <div onMouseDown={onZoomMouseDown} onDoubleClick={() => setZoom(1)} title="드래그로 확대/축소 · 더블클릭 리셋"
          className={`w-9 h-10 rounded-xl border backdrop-blur-md shadow-xl cursor-ns-resize flex flex-col items-center justify-center gap-[3px] transition-all hover:border-[#F6A823]/40 ${theme === 'light' ? 'bg-black/[0.04] border-black/10' : 'bg-white/[0.05] border-white/10'}`}>
