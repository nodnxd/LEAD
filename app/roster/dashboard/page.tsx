...
const ROLE_COLORS: Record<string, string> = {
  'Producer': '#DE6B35', 'Topliner': '#C49740', 'Engineer': '#3FA857', 'A&R': '#9C32C6'
};
...
;
  ...
  const exportAllDays = async (format: 'jpeg' | 'pdf' = 'jpeg') => {
    showToastMsg('📸 ' + (lang === 'ko' ? '생성 중...' : 'Generating...'));
    try {
      const ROLE_COLORS_MAP: Record<string, string> = { 'Producer': '#DE6B35', 'Topliner': '#C49740', 'Engineer': '#3FA857', 'A&R': '#9C32C6' };
      ...
      // 헤더
      ctx.font = 'bold 28px system-ui, sans-serif'; ctx.fillStyle = '#DE6B35';
      ...
      allDayData.forEach(({ dayLabel, teamData }, di) => {
        ...
        // Day 라벨
        ctx.font = 'bold 12px system-ui, sans-serif'; ctx.fillStyle = '#DE6B35';
        ...
        <main className={`min-h-screen ${bg} ${textMain} p-5 lg:p-8 font-pretendard relative overflow-hidden transition-colors duration-300`}>
          ...
          <DragDropContext onDragEnd={onDragEnd}>
                ...
                <div {...provided.droppableProps} ref={provided.innerRef} className="relative z-10 flex items-center gap-2 mb-6 overflow-x-auto pb-2 no-scrollbar">
                  {projects.map((p, index) => (
                    <Draggable key={p} draggableId={`proj-${p}`} index={index}>
                      {(provided) => (
                        <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                          className={`flex items-center rounded-full border transition-all overflow-hidden ${currentProject === p ? 'border-[#DE6B35]/50 bg-gradient-to-r from-[#DE6B35]/30 to-[#f3a17a]/10 shadow-[0_0_20px_rgba(222,107,53,0.3)]' : theme === 'light' ? 'border-black/10 bg-black/5' : 'border-white/10 bg-white/5'}`}>
                  ...
                  <button onClick={() => showPrompt(t.newRoster, t.rosterNamePlaceholder, '', async (n) => { if (n) { const next = [...projects, n]; setProjects(next); await saveProjectOrder(user.id, next); setCurrentProject(n); } setPromptModal(null); })}
                    className={`px-3 py-1.5 rounded-full font-bold text-[12px] border border-dashed hover:text-[#DE6B35] transition-all ${theme === 'light' ? 'bg-black/5 border-black/10 text-zinc-500' : 'bg-white/5 border-white/10 text-zinc-400'}`}>+</button>
                </div>
              )}
            </Droppable>
            ...
            <div className="relative z-10 flex items-center gap-2 mb-6">
              {days.map(d => (
                <div key={d} className={`flex items-center rounded-full border overflow-hidden transition-all ${currentDay === d ? 'border-[#DE6B35]/50 bg-gradient-to-r from-[#DE6B35]/20 to-transparent' : theme === 'light' ? 'border-black/10 bg-black/5' : 'border-white/10 bg-white/5'}`}>
                  {editingDayName === d ? (
                    <input autoFocus value={dayNameInput} onChange={e => setDayNameInput(e.target.value)}
                      ...
                      className="bg-transparent px-3 py-1.5 text-[11px] font-bold outline-none text-[#DE6B35] w-24" />
                  ) : (
                    <button onClick={() => setCurrentDay(d)} onDoubleClick={() => { setEditingDayName(d); setDayNameInput(dayNames[d] || `Day ${d}`); }}
                      className={`px-4 py-1.5 font-bold text-[11px] transition-all ${currentDay === d ? 'text-[#DE6B35]' : textSub}`}>{getDayLabel(d)}</button>
                  )}
              ...
              <button onClick={addDay} className={`px-3 py-1.5 rounded-full font-bold text-[12px] border border-dashed hover:text-[#DE6B35] transition-all ${theme === 'light' ? 'bg-black/5 border-black/10 text-zinc-500' : 'bg-white/5 border-white/10 text-zinc-400'}`}>{t.addDay}</button>
            </div>
                            ...
                            <div key={s.id} className={`rounded-2xl border overflow-hidden ${theme === 'light' ? 'border-black/10 bg-black/[0.02]' : 'border-white/10 bg-white/[0.02]'}`}>
                              ...
                              {expandedSession === s.id && s.roster && (
                                <div className={`px-4 pb-4 border-t pt-4 ${theme === 'light' ? 'border-black/5' : 'border-white/5'}`}>
                                  ...
                                  }} className="mb-3 px-3 py-1.5 rounded-lg bg-[#DE6B35]/20 text-[#DE6B35] text-[11px] font-bold border border-[#DE6B35]/30 hover:bg-[#DE6B35]/30 transition-all">
                            ...
                            {group.map(artist => {
                              ...
                              return (
                                <button key={artist.id} onClick={() => !inRoster && addArtistToRoster(artist)} disabled={inRoster}
                                  ...
                                  {inRoster ? <span className="text-[10px] text-zinc-600 font-black">✓</span> : <span className="text-[10px] text-[#DE6B35] font-black">+</span>}
                                </button>
                              );
                            })}
                          </div>
                        ...
                        {(provided) => (
                          <div ref={provided.innerRef} {...provided.draggableProps} className="w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] xl:w-[calc(25%-18px)]">
                            <div className={`backdrop-blur-2xl border rounded-[2rem] p-6 min-h-[400px] shadow-2xl flex flex-col ${cardBg}`}>
                              <div {...provided.dragHandleProps} className="flex justify-between items-start mb-4 px-1 border-l-4 border-[#DE6B35] pl-4 cursor-grab">
