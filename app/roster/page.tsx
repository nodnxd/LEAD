...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen bg-[#141414] text-white flex items-center justify-center font-pretendard relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-[-5%] w-[500px] h-[500px] rounded-full pointer-events-none" style={{background:'#5A9E7B', filter:'blur(200px)', animation:'orb-pulse 4s ease-in-out infinite'}} />

      <div className="relative z-10 w-full max-w-sm px-6">
        {/* 로고 */}
        <div className="text-center mb-10">
          <div className="flex items-baseline justify-center gap-2.5">
            <h1 className="text-6xl font-black text-[#5A9E7B] uppercase tracking-tighter">CAST</h1>
        ...
        {signupDone ? (
          <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-8 text-center">
            ...
            <button onClick={() => { setSignupDone(false); setAuthMode('login'); }} className="mt-6 text-[#5A9E7B] text-[12px] font-bold hover:underline">로그인 →</button>
          </div>
        ) : (
          <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6">
            <div className="flex gap-1 mb-6 bg-white/5 rounded-xl p-1">
              {(['login', 'signup'] as AuthMode[]).map(m => (
                <button key={m} onClick={() => { setAuthMode(m); setError(''); }} className={`flex-1 py-2 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all ${authMode === m ? 'bg-[#5A9E7B] text-white shadow-lg shadow-black/20' : 'text-zinc-500 hover:text-white'}`}>
                  {m === 'login' ? '로그인' : '회원가입'}
                </button>
              ))}
            </div>
            <div className="flex flex-col gap-3">
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="이메일" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[13px] outline-none focus:border-[#5A9E7B]/50 transition-all placeholder:text-zinc-600 text-white" />
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="비밀번호" onKeyDown={e => e.key === 'Enter' && handleAuth()} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[13px] outline-none focus:border-[#5A9E7B]/50 transition-all placeholder:text-zinc-600 text-white" />
              {authMode === 'login' && (
                <label className="flex items-center gap-2.5 cursor-pointer select-none group">
                  <div onClick={() => setRememberEmail(!rememberEmail)} className={`w-4 h-4 rounded-[4px] border transition-all flex items-center justify-center shrink-0 ${rememberEmail ? 'bg-[#5A9E7B] border-[#5A9E7B]' : 'border-white/20 bg-white/5 group-hover:border-white/40'}`}>
              ...
              <button onClick={handleAuth} disabled={loading} className="w-full bg-[#5A9E7B] text-white py-3 rounded-xl font-semibold text-[12px] uppercase tracking-widest hover:opacity-90 transition-all disabled:opacity-50 mt-1">
