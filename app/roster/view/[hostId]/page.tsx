      ...
      <div style={{ transform: `scale(${zoom})`, transformOrigin: 'top left', width: `${100 / zoom}%`, minHeight: `${100 / zoom}vh` }}>
        <main className={`min-h-screen ${bg} ${textMain} p-5 lg:p-8 font-ui relative overflow-hidden transition-colors duration-150`}>
          ...
          <a href="https://room-nu-seven.vercel.app" target="_blank" rel="noopener noreferrer"
            ...
            <div className="flex items-center justify-between gap-3 rounded-xl border px-4 py-2.5 backdrop-blur-md transition duration-200 border-[#ff2500]/25 hover:border-[#ff2500]/45"
              style={{ backgroundImage: 'linear-gradient(90deg, rgba(167,139,250,0.14), rgba(167,139,250,0.03) 55%, transparent)' }}>
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="inline-flex items-center justify-center w-6 h-6 rounded-full text-[#ff2500] bg-[#ff2500]/10 border border-[#ff2500]/25 shrink-0">
                ...
                <span className="text-body font-bold tracking-tight text-[#ff2500] shrink-0">room</span>
                <span className={`text-mini truncate ${textSub}`}>{lang === 'ko' ? '가사와 데모를 한곳에서' : 'lyrics & demos in one place'}</span>
              </div>
              <span className="flex items-center gap-1 text-mini font-semibold text-[#ff2500] whitespace-nowrap opacity-85 group-hover:opacity-100 transition-opacity shrink-0">
                {lang === 'ko' ? '열어보기' : 'open'}
