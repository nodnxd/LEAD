...
// 색상: gender(남자/여자/혼성) 기반, group_type(솔로/그룹)으로 dim
const getCardColor = (gender: string, group_type: string) => {
  const isGroup = group_type === 'group';
  if (gender === 'mixed') return {
    bg: isGroup ? 'bg-purple-500/10' : 'bg-purple-500/20',
    border: isGroup ? 'border-purple-500/20' : 'border-purple-500/40',
    text: isGroup ? 'text-purple-400/50' : 'text-purple-300',
    dot: isGroup ? 'bg-purple-400/35' : 'bg-purple-400',
    label: isGroup ? '혼성 그룹' : '혼성',
  };
...
const AlbumBadge = ({ type }: { type: string }) => {
  const t = ALBUM_MAP[type] || ALBUM_MAP.single;
  return <span className={`text-[9px] font-black px-1.5 py-0.5 rounded border ${t.cls}`}>{t.label}</span>;
};
...
const parseDeadline = (val: string) => {
  ...
  if (c.length === 4) return `${new Date().getFullYear()}-${c.slice(0,2)}-${c.slice(2,4)}`;
  return val;
};
...
const emptyForm = () => ({
  ...
  album_type: 'single', deadline: '', deadline2: '', content: '',
});
...
const DeadlineDisplay = ({ lead, size = 'normal' }: { lead: any; size?: 'compact'|'normal'|'large' }) => {
  ...
  if (d1 && d2) {
    ...
    const e1 = isExpired(d1); const e2 = isExpired(d2);
    if (size === 'compact') return (
      <div className="flex flex-col gap-0.5 ml-auto shrink-0">
        <span className={`text-[8px] font-black ${e1?'text-red-400/60':'text-zinc-700'}`}>1st {dd1}</span>
        <span className={`text-[9px] font-black ${e2?'text-red-400':'text-zinc-400'}`}>2nd {dd2}</span>
      </div>
    );
    if (size === 'large') return (
      <div className="flex flex-col items-end gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[12px] font-black px-2.5 py-0.5 rounded-full border ${e1?'text-red-400/60 border-red-500/20 bg-red-500/5':'text-zinc-500 border-zinc-700/60 bg-zinc-800/40'}`}>{dd1}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-zinc-300 text-[10px] font-black tracking-widest">2ND</span>
          <span className={`text-[15px] font-black px-3 py-0.5 rounded-full border ${e2?'text-red-400 border-red-500/30 bg-red-500/10':dd2==='D-DAY'?'text-yellow-400 border-yellow-500/30 bg-yellow-500/10':'text-zinc-100 border-zinc-500 bg-zinc-800/60'}`}>{dd2}</span>
        </div>
    ...
    return (
      <div className="flex flex-col items-end gap-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${e1?'text-red-400/60 border-red-500/20 bg-red-500/5':'text-zinc-600 border-zinc-700/50 bg-zinc-800/30'}`}>{dd1}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-zinc-400 text-[9px] font-black">2nd</span>
          <span className={`text-[12px] font-black px-2 py-0.5 rounded-full border ${e2?'text-red-400 border-red-500/30 bg-red-500/10':dd2==='D-DAY'?'text-yellow-400 border-yellow-500/30 bg-yellow-500/10':'text-zinc-300 border-zinc-600 bg-zinc-800/50'}`}>{dd2}</span>
        </div>
      </div>
    );
  }
  const deadline = d1||d2; const dday = getDDay(deadline); const expired = isExpired(deadline);
  const ddCls = expired?'text-red-400 border-red-500/30 bg-red-500/10':dday==='D-DAY'?'text-yellow-400 border-yellow-500/30 bg-yellow-500/10':'text-zinc-400 border-zinc-700 bg-zinc-800/50';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({data}) => {
      if (!data.user) router.push('/'); else setUser(data.user);
    });
  }, []);
  ...
  const fetchAnnouncements = async (u=user) => {
    if (!u) return;
    const {data} = await supabase.from('lead_announcements').select('*').eq('host_id',u.id).order('created_at',{ascending:true});
    if (data) setAnnouncements(data);
  };

  useEffect(() => { if (user) { fetchLeads(user); fetchAnnouncements(user); } }, [user]);

  const openCreate = (prefillDate?: string) => {
    const f = emptyForm(); if (prefillDate) f.deadline = prefillDate;
    setForm(f); setEditingLead(null); setShowModal(true);
  };
  const openEdit = (lead: any) => {
    setForm({
      title: lead.title, artist: lead.artist, gender: lead.gender||'male',
      group_type: lead.group_type||'solo', album_type: lead.album_type||'single',
      deadline: lead.deadline||'', deadline2: lead.deadline2||'', content: lead.content||'',
    });
  ...
  const saveLead = async () => {
    ...
    const payload = {
      ...
      group_type: form.group_type, album_type: form.album_type,
  ...
  const saveAnn = async () => {
    if (!annForm) return;
    if (annForm.id) {
      await supabase.from('lead_announcements').update({title:annForm.title,content:annForm.content,updated_at:new Date().toISOString()}).eq('id',annForm.id);
    } else {
      await supabase.from('lead_announcements').insert({host_id:user.id,title:annForm.title,content:annForm.content});
    }
    setAnnForm(null); fetchAnnouncements(); toast('📢 공지 저장됐어요!');
  };
  const deleteAnn = async (id: string) => {
    await supabase.from('lead_announcements').delete().eq('id',id);
    fetchAnnouncements(); toast('공지 삭제됐어요');
  };
  ...
  const insertLink = () => {
    ...
    const nc = form.content.slice(0,start)+ins+'\n'+form.content.slice(end);
    setForm(p=>({...p,content:nc}));
  ...
  const getLeadsForDay = (day: number) => {
    ...
    return leads.filter(l=>l.deadline===ds||l.deadline2===ds);
  };
  ...
  const DateShortcuts = ({field}:{field:'deadline'|'deadline2'}) => (
    <div className="flex gap-1.5 mt-2">
      {[1,3,7,14,30].map(d=>{
        ...
        return <button key={d} onClick={()=>setForm(p=>({...p,[field]:str}))}
          className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-zinc-500 text-[10px] font-bold hover:text-[#5B8CFF] hover:border-[#5B8CFF]/30 transition-all">+{d}일</button>;
      })}
    </div>
  );
  ...
  const LeadCard = ({lead,compact=false}:{lead:any;compact?:boolean}) => {
    ...
    return (
      <div onClick={()=>setViewingLead(lead)}
            ...
            <div className="flex items-start justify-between mb-2">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <AlbumBadge type={lead.album_type||'single'}/>
                </div>
    ...
    <>
      ...
      <main className="min-h-screen bg-[#050505] text-white p-5 lg:p-8 font-pretendard relative overflow-hidden">
        ...
        {announcements.length>0&&(
          <div className="relative z-10 mb-5 flex flex-col gap-2">
            {announcements.map(ann=>(
              <div key={ann.id} className="flex items-start gap-3 px-4 py-3 rounded-xl bg-[#5B8CFF]/10 border border-[#5B8CFF]/20">
                <span className="text-[#5B8CFF] text-[11px] font-black mt-0.5 shrink-0">📢</span>
                <div className="flex-1 min-w-0">
                  {ann.title&&<p className="text-white font-bold text-[13px] mb-0.5">{ann.title}</p>}
                  <p className="text-zinc-300 text-[12px] leading-relaxed whitespace-pre-line">{ann.content}</p>
                </div>
                <button onClick={()=>{setAnnForm({id:ann.id,title:ann.title||'',content:ann.content||''}); setShowAnnModal(true);}}
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-3 mb-6 border-b border-white/10 pb-4">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            <button onClick={()=>{setAnnForm({title:'',content:''}); setShowAnnModal(true);}}
      ...
      {viewingLead&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm font-pretendard p-4" onClick={()=>setViewingLead(null)}>
          <div className={`w-full max-w-2xl border rounded-[2rem] shadow-2xl ${getCardColor(viewingLead.gender,viewingLead.group_type).bg} ${getCardColor(viewingLead.gender,viewingLead.group_type).border}`}
            onClick={e=>e.stopPropagation()}>
            <div className="p-8 max-h-[85vh] overflow-y-auto">
              <div className="flex items-start justify-between mb-6">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-[11px] font-black ${getCardColor(viewingLead.gender,viewingLead.group_type).text}`}>{getCardColor(viewingLead.gender,viewingLead.group_type).label}</span>
                    <AlbumBadge type={viewingLead.album_type||'single'}/>
                  </div>
          ...
          <div className="w-full max-w-lg bg-[#111] border border-white/10 rounded-2xl shadow-2xl" onClick={e=>e.stopPropagation()}>
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {/* 기존 공지 목록 미니 */}
                {announcements.length>0&&!annForm?.id&&(
                  <span className="text-zinc-600 text-[11px]">현재 {announcements.length}개</span>
                )}
              </div>
                  ...
                  {announcements.map(ann=>(
                    <div key={ann.id} className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
                      ...
                      <div className="flex gap-2 shrink-0">
                        <button onClick={()=>setAnnForm({id:ann.id,title:ann.title||'',content:ann.content||''})}
                          className="text-zinc-500 hover:text-white text-[11px] font-bold transition-colors">수정</button>
                        <button onClick={()=>deleteAnn(ann.id)}
                          className="text-red-500/60 hover:text-red-400 text-[11px] font-bold transition-colors">삭제</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="border-t border-white/10 pt-4 flex flex-col gap-3">
                <p className="text-zinc-600 text-[11px]">{annForm?.id?'내용을 수정하세요':'새 공지를 작성하세요'}</p>
                <input value={annForm?.title||''} onChange={e=>setAnnForm(p=>p?({...p,title:e.target.value}):p)}
                  placeholder="제목"
                ...
                <textarea value={annForm?.content||''} onChange={e=>setAnnForm(p=>p?({...p,content:e.target.value}):p)}
            ...
            <div className="p-6">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input value={form.artist} onChange={e=>setForm(p=>({...p,artist:e.target.value}))} placeholder="아티스트명 *"
                  ...
                  <input value={form.title} onChange={e=>setForm(p=>({...p,title:e.target.value}))} placeholder="리드 제목 *"
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[13px] outline-none focus:border-[#5B8CFF]/50 transition-all placeholder:text-zinc-600 text-white"/>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="text-zinc-600 text-[10px] font-black uppercase tracking-widest mb-1.5 block">성별</label>
                    <select value={form.gender} onChange={e=>setForm(p=>({...p,gender:e.target.value}))}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[13px] outline-none text-zinc-300">
                  ...
                  <div>
                    <label className="text-zinc-600 text-[10px] font-black uppercase tracking-widest mb-1.5 block">타입</label>
                  ...
                  <div>
                    <label className="text-zinc-600 text-[10px] font-black uppercase tracking-widest mb-1.5 block">앨범</label>
                    <select value={form.album_type} onChange={e=>setForm(p=>({...p,album_type:e.target.value}))}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[13px] outline-none text-zinc-300">
                      <option value="single" className="bg-zinc-900">Single</option>
                      <option value="ep" className="bg-zinc-900">EP</option>
                      <option value="lp" className="bg-zinc-900">LP</option>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <input type="date" value={form.deadline} onChange={e=>setForm(p=>({...p,deadline:e.target.value}))}
                      className="bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-[13px] outline-none text-zinc-400 w-14 cursor-pointer"/>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button onClick={insertLink}
                      className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-zinc-400 text-[11px] font-bold hover:text-[#5B8CFF] hover:border-[#5B8CFF]/30 transition-all">🔗 링크 삽입</button>
                  </div>
