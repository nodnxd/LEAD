...
export default function LoginPage() {
  ...
  // 제품별 액센트
  const accent = isCast ? '#C48B94' : '#3E78DB';
  const accentHover = isCast ? '#B0757F' : '#2F62C2';
  const accentLight = isCast ? '#DCB2B8' : '#A9C4F0';
                ...
                {(['lead', 'cast'] as const).map(p => {
                  ...
                  return (
                    <button key={p} onClick={() => pickProduct(p)}
                      ...
                      style={on
                        ? { background: p === 'lead' ? '#3E78DB' : '#C48B94', color: '#fff' }
