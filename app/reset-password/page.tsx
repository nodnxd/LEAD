      ...
      <main className="min-h-screen bg-[#141414] flex items-center justify-center p-5 font-pretendard relative overflow-hidden">
        ...
        <div className="w-full max-w-sm relative z-10">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-black text-[#3E78DB] uppercase tracking-tighter">LEAD</h1>
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 shadow-2xl">
            {done ? (
              <div className="text-center py-4">
                ...
                <button onClick={() => router.push('/')} className="w-full py-3 rounded-xl bg-[#3E78DB] text-white font-semibold text-[13px] hover:opacity-90 transition-all">{t('로그인하러 가기', 'Go to login')}</button>
              </div>
            ) : !ready ? (
              <div className="text-center py-6">
                <div className="w-6 h-6 border-2 border-[#3E78DB] border-t-transparent rounded-full animate-spin mx-auto mb-3" />
              ...
              <>
                ...
                <div className="flex flex-col gap-3 mb-3">
                  <input value={password} onChange={e => setPassword(e.target.value)} placeholder={t('새 비밀번호', 'New password')} type="password"
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[14px] outline-none focus:border-[#3E78DB]/50 transition-all placeholder:text-zinc-600 text-white" />
                ...
                <button onClick={submit} disabled={loading} className="w-full py-3 rounded-xl bg-[#3E78DB] text-white font-semibold text-[13px] hover:opacity-90 transition-all disabled:opacity-50">
