        ...
        <div className="w-full max-w-sm relative z-10">
          ...
          {done && (
            <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-8 text-center">
              ...
              <p className="text-zinc-600 text-[12px] leading-relaxed mt-2">
                담당자 승인 후 이용하실 수 있어요.
              </p>
              <button
            ...
            <>
              ...
              <div className="flex bg-white/[0.03] border border-white/10 rounded-2xl p-1 gap-1 mb-5">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-[13px] font-bold transition-all ${mode === 'login' ? 'bg-white/10 text-white' : 'text-zinc-600 hover:text-zinc-400'}`}
                >
                  로그인
                </button>
                <button
                  onClick={() => { setMode('register'); setError(''); }}
                  className={`flex-1 py-2.5 rounded-xl text-[13px] font-bold transition-all ${mode === 'register' ? 'bg-white/10 text-white' : 'text-zinc-600 hover:text-zinc-400'}`}
                >
                  회원가입
                </button>
              </div>
                ...
                <div className="flex flex-col gap-3">
                  {mode === 'register' && (
                    <>
                      ...
                      <div>
                        <label className="text-zinc-600 text-[10px] font-black uppercase tracking-widest mb-1.5 block">전화번호 <span className="text-zinc-700 font-normal normal-case">(선택)</span></label>
                        <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="010-0000-0000"
                          className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-[13px] outline-none focus:border-[#5B8CFF]/50 transition-all placeholder:text-zinc-700 text-white"/>
                      </div>
