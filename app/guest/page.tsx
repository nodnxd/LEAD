          ...
          <div className={`border rounded-2xl overflow-hidden mb-5 ${card}`}>
            ...
            <div className={`p-6 border-b ${divider}`}>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {member?.roles?.map((r: string) => (
                      <span key={r} className="text-[9px] font-black px-2 py-0.5 rounded-full bg-[#80A1D4]/10 border border-[#80A1D4]/20 text-[#80A1D4]">{roleLabels[r] || r}</span>
                    ))}
                  </div>
                ...
                <button onClick={openEdit} className="shrink-0 px-3 py-2 rounded-xl bg-[#80A1D4]/10 border border-[#80A1D4]/20 text-[#80A1D4] text-[11px] font-bold hover:bg-[#80A1D4]/20 transition-all">
                  ✏️ 수정
                </button>
              </div>
            </div>
                        ...
                        <div className="flex flex-col gap-1.5 mt-2">
                          {files.map(f => (
                            <div key={f.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg ${D ? 'bg-black/20' : 'bg-black/[0.03]'}`}>
                              ...
                              {f.genre && <span className="text-[10px] font-black text-[#80A1D4]">{f.genre}</span>}
                            </div>
                          ))}
                        </div>
                      )}
            ...
            <div className={`w-full max-w-lg border rounded-2xl shadow-2xl my-4 ${D ? 'bg-[#1e1e1e] border-[rgba(255,255,255,0.08)]' : 'bg-white border-black/[0.08]'}`} onClick={e => e.stopPropagation()}>
              <div className="p-6 max-h-[85vh] overflow-y-auto">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[['male', '남성'], ['female', '여성'], ['other', '기타']].map(([v, l]) => (
                        <button key={v} onClick={() => setGender(v)} className={`flex-1 py-2 rounded-xl border text-[12px] font-bold transition-all ${gender === v ? 'bg-[#80A1D4]/20 border-[#80A1D4]/50 text-[#80A1D4]' : D ? 'bg-white/[0.03] border-white/[0.08] text-zinc-500' : 'bg-black/[0.03] border-black/[0.08] text-zinc-500'}`}>{l}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {ROLES.map(r => (
                        <button key={r.id} onClick={() => toggleArr(roles, r.id, setRoles)} className={`px-3 py-1.5 rounded-xl border text-[11px] font-bold transition-all ${roles.includes(r.id) ? 'bg-[#80A1D4]/20 border-[#80A1D4]/50 text-[#80A1D4]' : D ? 'bg-white/[0.03] border-white/[0.08] text-zinc-500' : 'bg-black/[0.03] border-black/[0.08] text-zinc-500'}`}>{r.label}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {GENRES.map(g => (
                        <button key={g.id} onClick={() => toggleArr(genres, g.id, setGenres)} className={`px-3 py-1.5 rounded-xl border text-[11px] font-bold transition-all ${genres.includes(g.id) ? 'bg-[#80A1D4]/20 border-[#80A1D4]/50 text-[#80A1D4]' : D ? 'bg-white/[0.03] border-white/[0.08] text-zinc-500' : 'bg-black/[0.03] border-black/[0.08] text-zinc-500'}`}>{g.label}</button>
                      ))}
                    </div>
                    {genres.includes('ETC') && (
                  ...
                  <div>
                    ...
                    <div className="mt-2"><label className={labelCls}>추가 데모 링크</label><input value={demoLink} onChange={e => setDemoLink(e.target.value)} placeholder="https://..." className={`w-full border rounded-xl px-3 py-2.5 text-[13px] outline-none transition-all ${inputCls}`} /></div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      {editWorks.map((w, i) => (
                        <div key={i} className={`p-3 rounded-xl border ${D ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-black/[0.02] border-black/[0.06]'}`}>
                          <div className="flex justify-between mb-2">
                            ...
                            {editWorks.length > 0 && <button onClick={() => setEditWorks(p => p.filter((_, idx) => idx !== i))} className="text-red-400/60 hover:text-red-400 text-[11px]">삭제</button>}
                          </div>
                      ...
                      {editWorks.length < 5 && (
                        <button onClick={() => setEditWorks(p => [...p, { song_title: '', artist_name: '', link: '' }])} className={`py-2 rounded-xl border border-dashed text-[11px] font-bold ${D ? 'border-white/10 text-zinc-600 hover:text-zinc-400' : 'border-black/10 text-zinc-400 hover:text-zinc-600'}`}>+ 추가</button>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button onClick={handleSave} disabled={saving} className="flex-1 py-3 rounded-xl bg-[#80A1D4] text-white font-semibold text-[13px] hover:opacity-90 transition-all disabled:opacity-50">
                    {saving ? '저장 중...' : '저장'}
                  </button>
                </div>
              </div>
